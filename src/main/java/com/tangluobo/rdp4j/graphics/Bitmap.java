...
public class Bitmap {
	 ...
	 */
	public Bitmap(State state, byte[] data, int width, int height, int x, int y, int Bpp) {
		this.highdata = Bitmap.convertImage(state, data, Bpp);
		this.width = width;
		this.height = height;
		this.x = x;
		this.y = y;
	...
	public static int convertTo24(State state, int colour) {
		if (state.getServerBpp() == 15)
			return convert15to24(colour);
		if (state.getServerBpp() == 16)
	...
	public static Image decompressImg(State state, int width, int height, int size, Packet data, int Bpp, IndexColorModel cm)
		...
		while (input < end) {
			...
			case 0: /* Fill */
				...
				break;
			case 8: /* Bicolor */
				color1 = cvalx(state, compressed_pixel, input, Bpp);
				// (compressed_pixel[input++]&0x000000ff);
				input += Bpp;
			case 3: /* Color */
				color2 = cvalx(state, compressed_pixel, input, Bpp);
				// color2 = (compressed_pixel[input++]&0x000000ff);
				input += Bpp;
				break;
			case 6: /* SetMix/Mix */
			case 7: /* SetMix/FillOrMix */
				// mix = compressed_pixel[input++];
				mix = cvalx(state, compressed_pixel, input, Bpp);
				input += Bpp;
				opcode -= 5;
				break;
			case 9: /* FillOrMix_1 */
				...
				case 4: /* Copy */
					while (((count & ~0x7) != 0) && ((x + 8) < width)) {
						for (int i = 0; i < 8; i++) {
							...
							// Bpp);
							w.setRGB(x, height, cvalx(state, compressed_pixel, input, Bpp));
							input += Bpp;
							count--;
							x++;
						}
				...
				case 4: /* Copy */
					while (((count & ~0x7) != 0) && ((x + 8) < width)) {
						for (int i = 0; i < 8; i++) {
							// pixel[line + x] = cvalx(compressed_pixel, input,
							// Bpp);
							w.setRGB(left + x, top + height, cvalx(state, compressed_pixel, input, Bpp));
							input += Bpp;
							count--;
							x++;
						}
				...
				case 4: /* Copy */
					while (((count & ~0x7) != 0) && ((x + 8) < width)) {
						for (int i = 0; i < 8; i++) {
							...
							// input, Bpp), Bpp);
							// pixel[line+x] = compressed_pixel[input++];
							pixel[line + x] = cvalx(state, compressed_pixel, input, Bpp);
							input += Bpp;
							count--;
							x++;
						}
	 ...
	 */
	public static int[] convertImage(State state, byte[] bitmap, int Bpp) {
		int[] out = new int[bitmap.length / Bpp];
		for (int i = 0; i < out.length; i++) {
			if (Bpp == 1)
				out[i] = bitmap[i] & 0xFF;
			else if (Bpp == 2)
				out[i] = ((bitmap[i * Bpp + 1] & 0xFF) << 8) | (bitmap[i * Bpp] & 0xFF);
			else if (Bpp == 3)
				out[i] = ((bitmap[i * Bpp + 2] & 0xFF) << 16) | ((bitmap[i * Bpp + 1] & 0xFF) << 8) | (bitmap[i * Bpp] & 0xFF);
			out[i] = Bitmap.convertTo24(state, out[i]);
		}
		return out;
	}
	 ...
	 * @return
	 */
	static int cvalx(State options, byte[] data, int offset, int Bpp) {
		int rv = 0;
