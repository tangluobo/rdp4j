...
public class FileCacheBackend implements CacheBackend {
	static Logger logger = LoggerFactory.getLogger(FileCacheBackend.class);
	public static final int MAX_CELL_SIZE = 0x1000; /* pixels */
	int g_pstcache_Bpp;
	boolean g_pstcache_enumerated = false;
	File[] g_pstcache_fd = new File[8];
	int g_stamp;
	private State state;
	...
	public void touchBitmap(int cache_id, int cache_idx, int stamp) {
	...
	public int pstcache_enumerate(State state, int cache_id, int[] idlist) throws IOException, RdesktopException {
		logger.info("PstCache.pstcache_enumerate");
		FileInputStream fd;
		int n, c = 0;
		CELLHEADER cellhdr = null;
		if (!(state.getOptions().isBitmapCaching() && state.getOptions().getPersistentCacheBackend() != null
				&& IS_PERSISTENT(cache_id)))
			return 0;
		/*
	...
	public boolean init(int cache_id) {
		// int fd;
		String filename;
		if (g_pstcache_enumerated)
			return true;
	...
	public boolean pstcache_load_bitmap(State state, int cache_id, int cache_idx) throws IOException, RdesktopException {
		...
		state.getCache().putBitmap(cache_id, cache_idx, bitmap, c.stamp);
		// xfree(celldata);
		return true;
	}

	/* Store a bitmap in the persistent cache */
	public boolean pstcache_put_bitmap(State state, int cache_id, int cache_idx, byte[] bitmap_id, int width, int height,
