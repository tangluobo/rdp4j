...
import com.tangluobo.rdp4j.orders.DeskSaveOrder;
import com.tangluobo.rdp4j.orders.DestBltOrder;
import com.tangluobo.rdp4j.orders.LineOrder;
import com.tangluobo.rdp4j.orders.MemBltOrder;
import com.tangluobo.rdp4j.orders.PatBltOrder;
import com.tangluobo.rdp4j.orders.Pen;
import com.tangluobo.rdp4j.orders.PolyLineOrder;
import com.tangluobo.rdp4j.orders.RectangleOrder;
...
public class Orders {
	...
	private void process_bmpcache2(Packet data, int flags, boolean compressed) throws RdesktopException, IOException {
		...
		if (compressed) {
			if (Bpp == 1)
				bmpdataInt = Bitmap.convertImage(state, Bitmap.decompress(width, height, bufsize, data, Bpp), Bpp);
			else
			...
			for (y = 0; y < height; y++)
				data.copyToByteArray(bmpdata, y * (width * Bpp), (height - y - 1) * (width * Bpp), width * Bpp); // memcpy(&bmpdata[(height
			// - y -
			// 1) *
			// (width
			// *
			...
			// *
			// Bpp);
			bitmap = new Bitmap(Bitmap.convertImage(state, bmpdata, Bpp), width, height, 0, 0);
		}
