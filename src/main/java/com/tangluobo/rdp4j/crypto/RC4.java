package com.tangluobo.rdp4j.crypto;
...
public final class RC4 { // must be final for security reasons
	...
	protected int engineUpdate(byte[] in, int inOffset, int inLen, byte[] out, int outOffset) {
		...
		if (in == out
			&& (outOffset >= inOffset && outOffset < inOffset + inLen || inOffset >= outOffset && inOffset < outOffset + inLen)) {
	 ...
	 *                </ul>
	 */
	private void makeKey(byte[] userkey) throws RdesktopCryptoException {

		if (userkey == null)
			throw new RdesktopCryptoException(getAlgorithm() + ": Null user key");

		int len = userkey.length;
		if (len == 0)
			throw new RdesktopCryptoException(getAlgorithm() + ": Invalid user key length");
